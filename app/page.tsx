...
//   if (signals.cognitiveLoad < 20 && signals.attention > 80)
//     return "😌 Relaxed and Alert";
//   if (signals.focus > 60 && signals.cognitiveLoad > 60)
//     return "🤔 Concentrating Hard";
//   if (signals.attention > 80 && signals.stress < 30)
//     return "😊 Calmly Attentive";
...
//     () => new Tone.Filter(1000, "lowpass").toDestination(),
//     []
//   );
...
//           lastFocusState.current = currentFocusState;
//         }
//       }
...
//           <div>Attention: {signals.attention.toFixed(2)}%</div>
//           <div>Stress: {signals.stress.toFixed(2)}%</div>
//           <div>Focus: {signals.focus.toFixed(2)}%</div>
//           <div>Cognitive Load: {signals.cognitiveLoad.toFixed(2)}%</div>
//           <div>Emotion: {signals.emotion}</div>
//           <div>Mental Fatigue: {signals.mentalFatigue.toFixed(2)}%</div>
//         </div>
//       </Html>
//     </group>
//   );
...
//                   setSignals((prev) => ({
//                     ...prev,
//                     [signalType]: parseFloat(e.target.value),
//                   }))
//                 }
//                 disabled={isAutoMode}
...
//           <li>
//             Sphere size reflects attention level (larger = more attentive)
//           </li>
...
//           <li>
//             Text and emoji above sphere describes the current mental state
//           </li>
...
//           focus: Math.max(
//             0,
//             Math.min(100, prevSignals.focus + (Math.random() - 0.5) * 20)
//           ),
//           cognitiveLoad: Math.max(
//             0,
//             Math.min(
//               100,
//               prevSignals.cognitiveLoad + (Math.random() - 0.5) * 20
//             )
//           ),
//           stress: Math.max(
//             0,
//             Math.min(100, prevSignals.stress + (Math.random() - 0.5) * 20)
//           ),
//           attention: Math.max(
//             0,
//             Math.min(100, prevSignals.attention + (Math.random() - 0.5) * 20)
//           ),
//           emotion: prevSignals.emotion,
//           mentalFatigue: Math.max(
//             0,
//             Math.min(
//               100,
//               prevSignals.mentalFatigue + (Math.random() - 0.5) * 10
//             )
//           ),
//         }));
//       }, 1000);
...
const WebcamEmotionDetector: React.FC<WebcamEmotionDetectorProps> = ({
  ...
  const handleVideoPlay = () => {
    ...
    const detectionInterval = isMobile ? 500 : 100; // Reduce detection frequency on mobile
